...
full.table = read.csv("OUTPUTS/WATCHLIST_SYN_COLLATED.csv")
...
for(t in thresh){
  ...
  # save this list
  full.table.thresh = dplyr::filter(full.table, prop_records_in_kg >= t & 
                                      total_n >= 50)
  ...
  climate.bars = ggplot(data = full.table.thresh.long, 
    ...
    scale_fill_manual(values = c("black", "royalblue", "yellow", 
                                 "purple")) +
    labs(title = paste0("KG similarity >= ", t, "% (n = ", num.spp.thresh, ")"),
         x = "Species",
  ...
  ggsave(plot = climate.bars, 
         filename = paste0("OUTPUTS/watchlist_", t, ".png"), 
         dpi = 450, height = 7, width = 10)
